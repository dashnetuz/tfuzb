...
use common\models\QuizEssaySubmission;
use common\models\UserPartProgress;
...
//QUIZ
    ...
    public function actionCourseView($id)
    {
        ...
        if ($userId) {
            foreach ($lessons as $lesson) {
                ...
                $incompleteParts = array_filter($parts, function ($part) use ($userId) {
                    $quizPassed = true;
                    $essaySubmitted = true;
                    ...
                    // Final check
                    return !($quizPassed && $essaySubmitted);
                });

                // Agar hech bo‘lmaganda 1ta yakunlanmagan part bo‘lsa, bu dars aktiv hisoblanadi
    ...
    public function actionLessonView($id)
    {
        ...
        $isLessonFullyCompleted = count($parts) > 0;

        if ($userId) {
            $completedPartIds = (new \yii\db\Query())
                ->select('part_id')
            ...
            foreach ($parts as $part) {
                ...
                $partQuizzesOk = true;
                $isPartCompleted = in_array($part->id, $completedPartIds);

                foreach ($quizzes as $quiz) {
                    $stats = $quiz->getAttemptStats($userId);
                    $quizAttempts[] = array_merge($stats, [
                        'title' => $quiz->title,
                        'type' => $quiz->type,
                        'part_id' => $part->id,
                        'lesson_id' => $lesson->id,
                    ...
                    // Eski `quizStatuses` va `essayStatuses` o‘rniga agar kerak bo‘lsa yozing:
                    if ($quiz->type === 'test') {
                        $quizStatuses[$part->id] = $stats['status'];
                    } elseif ($quiz->type === 'essay') {
                        $essayStatuses[$part->id] = $stats['status'];
                    }
                }

                if ($partQuizzesOk && !in_array($part->id, $completedPartIds)) {
                    // ❗Agar avtomatik yakunlashni istasangiz, shu yerda yozishingiz mumkin:
                    // $completedPartIds[] = $part->id;
                } else {
                    $isLessonFullyCompleted = false;
                }
                if (!$isPartCompleted || !$partQuizzesOk) {
                    $isLessonFullyCompleted = false;
                }
            }
        ...
        return $this->render('lesson-view', [
            ...
            'essayStatuses' => $essayStatuses,
            'quizAttempts' => $quizAttempts,
            'isLessonFullyCompleted' => $isLessonFullyCompleted,
        ]);
    }
    public function actionLoadPart($id, $index = 0)
    {
        Yii::$app->response->format = Response::FORMAT_JSON;

        $part = \common\models\Part::findOne(['id' => $id]);
        if (!$part) {
            return ['success' => false, 'message' => 'Part topilmadi.'];
        }
        ...
        if ($userId) {
            $isCompleted = (new \yii\db\Query())
                ->from('{{%user_part_progress}}')
                ->where(['user_id' => $userId, 'part_id' => $part->id, 'is_completed' => true])
                ->exists();
            ...
            if ($part->test) {
                ...
                } elseif (!$lastAttempt['is_passed']) {
                    $quizStatus = 'failed';
                } else {
                    $quizStatus = 'passed';
                    $attemptInfo = [
                        'score' => $lastAttempt['score'] ?? 0,
                        'total' => $lastAttempt['total_questions'] ?? 0,
                    ];
                }
            }
            ...
            if ($part->essay) {
                ...
                if ($submission) {
                    $essayStatus = is_null($submission['score']) ? 'pending' : 'submitted';
                } else {
                    $essayStatus = 'not_submitted';
                }
            }
        }

        // 💡 1. Barcha kontentlarni tartib bilan olish
        $contents = $part->getSortedContents(); // bu metod `orderBy(position)` bilan yozilgan deb faraz qilinmoqda

        // 💡 2. Nechanchi kontentni ko‘rsatish kerakligini aniqlash
        $index = (int) $index;
        if (!isset($contents[$index])) {
            return ['success' => false, 'message' => 'Kontent mavjud emas.'];
        }

        // 💡 3. Keyingi Part topish (darsga tegishli partlar ro‘yxatidan)
        $nextPartId = null;
        $lessonParts = $part->lesson->getParts()
            ->where(['is_active' => true])
            ->orderBy(['position' => SORT_ASC])
            ->all();

        $currentPartIndex = array_search($part->id, array_column($lessonParts, 'id'));
        if ($currentPartIndex !== false && isset($lessonParts[$currentPartIndex + 1])) {
            $nextPartId = $lessonParts[$currentPartIndex + 1]->id;
        }

        // 💡 4. Render qilish
        $html = $this->renderPartial('_part_ajax', [
            'part' => $part,
            'contents' => $contents,
            'currentIndex' => $index,
    ...
    public function actionMarkPartComplete()
    {
        ...
        try {
            ...
            $userId = $this->user['id'] ?? Yii::$app->user->id ?? null;

            if (!$partId || !$userId) {
                $msg = 'Foydalanuvchi yoki bo‘lim ID topilmadi.';
                Yii::$app->session->setFlash('error', $msg);
                return ['success' => false, 'message' => $msg];
            }

            $part = Part::find()->with(['test', 'essay'])->where(['id' => $partId])->one();
            if (!$part) {
            ...
            // 1. Test tekshiruv
            if ($part->test && $part->test->id) {
                $attempt = (new \yii\db\Query())
                    ->from('quiz_attempt')
                    ->where(['user_id' => $userId, 'quiz_id' => $part->test->id])
                    ->orderBy(['id' => SORT_DESC])
                    ->one();
                ...
                if (!$attempt) {
                    $msg = 'Testni ishlamadingiz.';
                    Yii::$app->session->setFlash('error', $msg);
                    return ['success' => false, 'message' => $msg];
                }

                $score = $attempt['score'] ?? 0;
                $required = $attempt['quiz']['required_percent'] ?? null;

                if (!$attempt['is_passed']) {
                    $msg = 'Testdan o‘ta olmadingiz. Natija: ' . $score . '%';
                    if ($required !== null) {
                        $msg .= ', talab: ' . $required . '%';
                    }
                    Yii::$app->session->setFlash('error', $msg);
            ...
            // 2. Essay tekshiruv
            if ($part->essay && $part->essay->id) {
                $submission = (new \yii\db\Query())
                    ->from('quiz_essay_submission')
                    ->where([
                        'user_id' => $userId,
                        'part_id' => $partId,
                        'quiz_id' => $part->essay->id,
                        'is_submitted' => true,
                    ])
                    ->one();

                if (!$submission) {
                    $msg = 'Esseni topshirmadingiz.';
                    Yii::$app->session->setFlash('error', $msg);
                    return ['success' => false, 'message' => $msg];
                }

                if (is_null($submission['score'])) {
                    $msg = 'Esseni natijasi hali tekshirilmagan.';
                    Yii::$app->session->setFlash('info', $msg);
                    return ['success' => false, 'refresh' => true]; // 👈 reload qilish uchun
                }

            }

            // 3. Kontentni oxirigacha o‘qish tekshiruvi (test/essay bo‘lmaganlar uchun)
            if (!$part->test && !$part->essay) {
                $totalContents = count($part->getSortedContents());
                if (!is_numeric($index) || ((int)$index + 1) < $totalContents) {
                    $msg = 'Iltimos, kontentni oxirigacha o‘qing.';
                    Yii::$app->session->setFlash('error', $msg);
                    return ['success' => false, 'message' => $msg];
                }
            }

            // 4. Yozuv bor-yo‘qligini tekshirish
            ...
            if (!$exists) {
                Yii::$app->db->createCommand()->insert('user_part_progress', [
                    'user_id' => $userId,
                    'part_id' => $partId,
                    'is_completed' => true,
            ...
            Yii::$app->session->setFlash('success', 'Bo‘lim muvaffaqiyatli yakunlandi.');
            return ['success' => true];
        } catch (\Throwable $e) {
            // 🔴 Kutilmagan xato
            $errorMsg = 'Bo‘limni yakunlashda ichki xatolik: ' . $e->getMessage();
            Yii::error("[PartCompleteError] " . $e->getMessage() . "\n" . $e->getTraceAsString(), 'part.complete');
            Yii::$app->session->setFlash('error', $errorMsg);
            return ['success' => false, 'message' => $errorMsg];
        }
    }

    public function completePart($userId, $partId)
    {
        $progress = UserPartProgress::findOne(['user_id' => $userId, 'part_id' => $partId]);

        if (!$progress) {
            $progress = new UserPartProgress();
            $progress->user_id = $userId;
            $progress->part_id = $partId;
        }

        $progress->is_completed = true;
        $progress->completed_at = date('Y-m-d H:i:s');
        $progress->save(false); // validatsiya bo'lmasa
    }
    public function isPartCompleted($userId, $partId)
    {
        return UserPartProgress::find()
            ->where(['user_id' => $userId, 'part_id' => $partId, 'is_completed' => true])
            ->exists();
    }
    public function actionTestStart($id)
    {
        ...
        // Max attempt check
        $attemptCount = QuizAttempt::find()->where(['quiz_id' => $quiz->id, 'user_id' => $userId])->count();
        if ($quiz->max_attempt && $attemptCount >= $quiz->max_attempt) {
            Yii::$app->session->setFlash('error', 'Siz maksimal urinishlar soniga yetgansiz.');
            return $this->redirect(['dashboard/part-view', 'id' => $quiz->part_id]);
        }
        ...
        if (Yii::$app->request->isPost) {
            ...
            foreach ($questions as $question) {
                $selectedOptionId = $post['question'][$question->id] ?? null;

                $option = QuizOption::findOne($selectedOptionId);
            ...
            $isPassed = $score >= $quiz->pass_percent;

            $attempt->ended_at = date('Y-m-d H:i:s');
            $attempt->score = $score;
    ...
    public function actionEssaySubmit($part_id)
    {
        $part = Part::findOne($part_id);
        if (!$part || !$part->essay) {
            throw new NotFoundHttpException('Bo‘lim yoki essay topilmadi.');
        }

        $userId = $this->user['id'] ?? null;
        if (!$userId) {
            throw new ForbiddenHttpException('Avval tizimga kiring.');
        }

        $model = QuizEssaySubmission::findOne([
            'user_id' => $userId,
            'part_id' => $part_id,
            'quiz_id' => $part->essay->id,
        ]);
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            // Faqat yangi yozuv yoki birinchi marta topshirilayotgan bo‘lsa vaqt yozamiz
            if ($isNew || $model->submitted_at === null) {
                $model->submitted_at = date('Y-m-d H:i:s');
            }

            if ($model->save()) {
                Yii::$app->session->setFlash('success', 'Essay muvaffaqiyatli yuborildi!');
                return $this->redirect(['/dashboard/lesson/view/' . $part->lesson_id]);
            }
        }

        return $this->render('essay-submit', [
            'part' => $part,
            'model' => $model,
        ]);
    }
    public function actionCheckTestCompleted($part_id)
    {
        Yii::$app->response->format = Response::FORMAT_JSON;

        $userId = $this->user['id'] ?? null;
        if (!$userId || !$part_id) {
            return ['success' => false, 'message' => 'Foydalanuvchi yoki bo‘lim aniqlanmadi.'];
        }

        $part = Part::findOne(['id' => $part_id]);
        if (!$part) {
            return ['success' => false, 'message' => 'Bo‘lim topilmadi.'];
        }
        ...
        if ($part->test) {
            ...
            if (!$attempt) {
                return ['success' => false, 'message' => 'Iltimos, testni yeching.'];
            }

            if (!$attempt['is_passed']) {
                return ['success' => false, 'message' => 'Siz testdan o‘ta olmadingiz. Qayta urinib ko‘ring.'];
            }
        }
        ...
        if ($part->essay) {
            ...
            if (!$submission) {
                return ['success' => false, 'message' => 'Iltimos, esse topshiring.'];
            }

            if (is_null($submission['score'])) {
                return ['success' => false, 'message' => 'Essening natijasi hali baholanmadi.'];
            }
        }
