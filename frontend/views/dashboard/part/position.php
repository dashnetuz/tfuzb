...
/** @var common\models\Part $lesson_id */

$lessonId = Yii::$app->request->get('lesson_id');

$this->title = Yii::t('app', 'Qismlar ro‘yxatini tartibga solish');
...
$this->registerJs(<<<JS
    $('#nestable-part').nestable();

    $('#save-order').on('click', function () {
        let order = $('#nestable-part').nestable('serialize');
        $.ajax({
            url: 'update-position?model=part',
            type: 'POST',
            data: {order: order},
...
<div class="datatables">
    <div class="card">
        <div class="card-body">
            ...
            <?= Html::a(Yii::t('app', 'Yangi qo‘shish'), ['dashboard/part-create', 'lesson_id' => $lessonId], [
                'class' => 'btn btn-primary text-nowrap mb-4'
            ]) ?>

            <?= Html::a(Yii::t('app', 'Ro‘yxatga qaytish'), ['dashboard/part-index', 'lesson_id' => $lessonId], [
                'class' => 'btn btn-secondary text-nowrap mb-4'
            ]) ?>

            <div class="dd" id="nestable-part">
                <ol class="dd-list">
